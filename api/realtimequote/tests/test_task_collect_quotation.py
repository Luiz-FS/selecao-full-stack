...
class TestCollectCoinQuotationHistory:
    ...
    def test_when_expect_success(self, mocker, coin_to_backend_api_mock, backend_api_mock):
        # arrange
        coin = Coin()
        quotation = QuotationSchema(min_price=Decimal(5.00), max_price=Decimal(5.64), variance=Decimal(0.2), create_date=datetime.now().date())

        mock_get_quote_history = mocker.patch.object(backend_api_mock, "get_quote_history", return_value=[quotation])
        mock_get_coin_by_name = mocker.patch("apps.coin.tasks.CoinRepository.get_coin_by_name", return_value=coin)
        mock_bulk_create_quotation = mocker.patch("apps.quotation.tasks.QuotationRepository.bulk_create_quotation")
    ...
    def test_when_occours_request_exception(self, mocker, coin_to_backend_api_mock, backend_api_mock):
        # arrange
        mock_get_quote_history = mocker.patch.object(backend_api_mock, "get_quote_history", side_effect=RequestExeption())
    ...
    def test_when_occours_unexpected_exception(self, mocker, coin_to_backend_api_mock, backend_api_mock):
        # arrange
        mock_get_quote_history = mocker.patch.object(backend_api_mock, "get_quote_history", side_effect=Exception())
        mock_get_coin_by_name = mocker.patch("apps.coin.tasks.CoinRepository.get_coin_by_name")
