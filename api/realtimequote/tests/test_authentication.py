...
class TestIsAuthenticated:
    def test_authenticate_sucess(self, mocker, request_mock, response_mock):
        ...
        mock_get.assert_called_with(
            f"{settings.AUTHENTICATOR_URI}/auth/verify?token={request.headers['Authorization']}"
        )
    ...
    def test_authenticate_token_already_exists(self, mocker, request_mock):
        ...
        request = request_mock(
            headers={"Authorization": "48309809384349089023"},
            session={"token": "48309809384349089023", "user": user_data},
        )
