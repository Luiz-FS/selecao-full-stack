...
class TestCollectCoinQuotation:
    def test_when_expect_success(self, mocker, coin_to_backend_api_mock, backend_api_mock):
        ...
        mock_get_current_quote = mocker.patch.object(
            backend_api_mock, "get_current_quote", return_value=coin
        )
        mock_update_current_quote = mocker.patch(
            "apps.coin.tasks.CoinRepository.update_current_quote"
        )
    ...
    def test_when_occours_request_exception(
        ...
        # arrange
        mock_get_current_quote = mocker.patch.object(
            backend_api_mock, "get_current_quote", side_effect=RequestExeption()
        )
